...
function ServiceSlotCart({ publicServiceResponse, publicBookableLocation, checkUserLogin, user_id, locationId, serviceId }) {
    ...
    const handleDateCLick = (event, date_html_format, resource_required) => {
        ...
        const api_service_id = publicServiceResponse?.biz_service_id;
    ...
    const add_to_cart = async (e) => {
        // if (localStorage.getItem('role') !== 'vendor') {
            ...
            FORM_DATA.append('service_id', publicServiceResponse.biz_service_id);
            FORM_DATA.append('package_id', 0);
            FORM_DATA.append('resource_id', resource_id);
            if(localStorage.getItem('role')!=='vendor'){
                FORM_DATA.append('user_id', user_id);
            }else{
                FORM_DATA.append('customer_email', customerEmail);
            }
            ...
            if (parseInt(publicServiceResponse?.engine) === 3) {
                if (selected_slot.length === sessionDataDetails?.session_dates.length) {
                    if (parseInt(publicServiceResponse?.engine) === 5 || parseInt(publicServiceResponse?.engine) === 6) {
                        ...
                        if (publicServiceResponse?.check_service_in_package?.order_id) {
                            ...
                            // console.log("ADD_TO_CART======>",result);
                            if (result?.response?.data?.status === 422) {
                                setApiErrors(result?.response?.data?.errors);
                                setButtonLoader(false);
                ...
                if (parseInt(publicServiceResponse?.engine) === 5 || parseInt(publicServiceResponse?.engine) === 6) {
                    ...
                    if (publicServiceResponse?.check_service_in_package?.order_id) {
                        ...
                        const result = await ADD_TO_CART(FORM_DATA);
                        if (result?.response?.data?.status === 422) {
                            console.log("ADD_TO_CART-2",result);
                            setApiErrors(result?.response?.data?.errors);
                            setButtonLoader(false);
                            ...
                            {checkUserLogin ? <>
                                {successMsg ? <>
                                    <div className='login-para'>
                                        ...
                                        {publicServiceResponse?.engine == 5 || parseInt(publicServiceResponse?.engine) == 6 ? <>
                                    ...
                                    <div className='biz-col-5'>
                                        <div className='bg-color-box'>
                                            {selected_slot?.length > 0 ? <>
                                                {publicServiceResponse?.engine === 5 || publicServiceResponse?.engine === 6 ? <>
                                                    <div className='cart-row'>
                                                        ...
                                                        {localStorage.getItem('role') === 'vendor'?
                                                            <div className='biz-row'>
                                                                <div className='biz-col-12'>
                                                                    <label style={{ margin: "5px 0px", display: "block" }}>Enter Customer Email</label>
                                                                    <input type='email' onChange={(e) => setCustomerEmail(e.target.value)} className='biz-input'/>
                                                                </div>
                                                                {apiErrors && apiErrors?.customer_email && (
                                                                  <span style={{ color: "red", paddingLeft:'20px' }}>
                                                                    {apiErrors?.customer_email[0]}
                                                                  </span>
                                                                )}
                                                            </div>
                                                        :<></>}
                                                        ...
                                                        <div className='biz-row'>
                                                            ...
                                                            <div className='biz-col-9'>
                                                                {publicServiceResponse?.engine === 6 ? '' : <>
                                                    ...
                                                    <div className='biz-row' style={{ margin: "0px -15px" }}>
                                                        <div className='biz-col-12'>
                                                            {buttonLoader ? <>
                                                                <span className='biz-black-btn'><Spin indicator={<LoadingOutlined style={{ fontSize: 15 }} spin />} /> {publicServiceResponse?.engine === 6 ? 'Book Service' : 'Book Appointment'}</span>
                                                            </> : <>
                                                                <span className='biz-black-btn' onClick={(e) => add_to_cart(e)}>{publicServiceResponse?.engine === 6 ? 'Book Service' : 'Book Appointment'}</span>
                                                            </>}
                                                    ...
                                                    <div className='cart-row'>
                                                        ...
                                                        <div className='biz-row'>
                                                            ...
                                                            <div>
                                                                <div className='biz-col-9' style={{marginLeft:'-25px'}} >
                                                                    <h3 style={{paddingLeft:'25px'}}>{publicServiceResponse?.title}</h3>
                                                                </div>
                                                                <div >
                                                                    <h4 style={{display: 'flex', justifyContent: 'flex-end',paddingRight:'15px'}}>${parseFloat(priceBox?.price).toFixed(2)}</h4>
                                                                </div>
                                                            </div>
                                                                {selected_slot?.map(item => (<>
                                                                    {publicServiceResponse?.engine === 3 && <>
                                                                        <h6 style={{ marginTop: '20px', paddingLeft:'15px', color: '#ED510C' }}>{item.date} | {item.start_time} - {item.end_time} | {publicBookableLocation?.duration} Min.</h6>
                                                                    </>}
                                                                    {publicServiceResponse?.engine === 1 && <>
                                                                        <h6 style={{ marginTop: '20px', paddingLeft:'15px', color: '#ED510C' }}>{item.date} | {item.start_time} - {item.end_time} | {publicBookableLocation?.duration} Min.</h6>
                                                                    </>}
                                                                    {publicServiceResponse?.engine === 2 && <>
                                                                        <h6 style={{ paddingLeft:'15px'}}> {item.start_time} - {item.end_time} | {publicBookableLocation?.duration} Min.</h6>
                                                    ...
                                                    {resource_required === 1 && <>
                                                        ...
                                                        <div className='biz-row'>
                                                            ...
                                                            <div className="biz-col-6" style={{ textAlign: "left", color: '#ED510C' }}>
                                                                {resource_name}
                                                            </div>
                                                        </div>
                                                    </>}
                                                    <div className='cart-total'>
                                                        ...
                                                        {publicServiceResponse?.checkout_price_array.discount != "0" && publicServiceResponse?.checkout_price_array.discount ?
                                                        <div className='biz-row'>
                                                            <div className='biz-col-6'>
                                                                <span>Discount </span>
                                                            </div>
                                                            ...
                                                            <div className='biz-col-6' style={{ textAlign: "right" }}>
                                                                <span>{ parseInt(publicServiceResponse?.checkout_price_array.discount)}%</span>
                                                            </div>
                                                        </div>:
                                                        <></> }
                                                    </div>
                                ...
                                <div className='login-para'>
                                    ...
                                    <span className='biz-orange-btn' onClick={() => {
                                        router.push('/login?route=service-detail&service_id=' + publicServiceResponse?.biz_service_id + '&location_id=' + publicBookableLocation?.location_id)
                                    }}>Login </span>
                                </div>
